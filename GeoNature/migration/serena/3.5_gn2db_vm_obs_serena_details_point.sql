...
DROP MATERIALIZED VIEW IF EXISTS _import_serena.vm_obs_serena_detail_point cascade;
CREATE MATERIALIZED VIEW _import_serena.vm_obs_serena_detail_point AS
WITH
    obs_obsv AS (
        /* Liste des Observateurs avec id et noms utilisés */
        SELECT
            q.obse_obsv_id
          , rnf_srce.srce_compnom_c AS observateur
            FROM
                (SELECT DISTINCT
                     o.obse_obsv_id
                     FROM
                         _import_serena.rnf_obse o) q
                    JOIN _import_serena.rnf_srce ON q.obse_obsv_id = rnf_srce.srce_id
    )
  , obs_deter AS (
    SELECT
        /* Liste des déterminateurs avec id et nom utilisés */
        q.obse_detm_id
  ...
  , obs_taxon AS (
    /* liste des taxons avec id et noms utilisés*/
    SELECT
        q.taxo_id
      , q.nom_vern  AS nom_verna
      , q.nom_latin
      , q.nom_latin AS nom_valide
        FROM
            (SELECT
                 t1.taxo_id
               , t1.taxo_vernacul AS nom_vern
               , t1.taxo_latin_c  AS nom_latin
               , t1.taxo_synonym_id
                 FROM
                     (SELECT DISTINCT
                          o.obse_taxo_id
                          FROM
                              _import_serena.rnf_obse o) q1
                         JOIN _import_serena.rnf_taxo t1 ON q1.obse_taxo_id = t1.taxo_id) q
                LEFT JOIN _import_serena.rnf_taxo t2 ON q.taxo_synonym_id = t2.taxo_id
)
  , obs_prot AS (
    /* liste des protocoles utilisés*/
    SELECT
        q.obse_pcole_choi_id
  ...
  , obs_valid AS (
    /* statuts de validations utilisés */
    SELECT
        q.obse_validat_choi_id
  ...
  , obs_confid AS (
    /* statuts de confidentialité utilisés */
    SELECT
        q.obse_confid_choi_id
  ...
  , obs_age_unite AS (
    SELECT
        q.obse_ageunit_choi_id
      , rnf_choi.choi_nom AS unite
        FROM
            (SELECT DISTINCT
                 o.obse_ageunit_choi_id
                 FROM
                     _import_serena.rnf_obse o) q
  ...
  , obs_abond AS (
    SELECT
        q.obse_abond_choi_id
      , rnf_choi.choi_nom AS abondance
        FROM
            (SELECT DISTINCT
                 o.obse_abond_choi_id
                 FROM
                     _import_serena.rnf_obse o) q
  ...
  , obs_methode_loc AS (
    SELECT
        q.obse_methloc_choi_id
      , rnf_choi.choi_nom AS methode_loc
        FROM
            (SELECT DISTINCT
                 o.obse_methloc_choi_id
                 FROM
                     _import_serena.rnf_obse o) q
  ...
  , obs_geom AS (
    SELECT
      ...
      , 'observation' as geom_type
        FROM
            _import_serena.rnf_obse o
        WHERE
              o.geom_obse IS NOT NULL
...
SELECT
  ...
  , CASE
        WHEN (substr(rnf_obse.obse_date::TEXT, 5, 4) ~ '^([0-9]+[.]?[0-9]*|[.][0-9]+)$'::TEXT) = TRUE
            THEN to_date("left"(rnf_obse.obse_date::TEXT, 8), 'YYYYMMDD'::TEXT)
  ...
  , obs_abond.abondance
  , CASE
        WHEN rnf_obse.obse_nombre LIKE '%,%'
            THEN (SELECT
                      min(c)::INT
                      FROM
                          (SELECT n1::INT AS c FROM unnest(string_to_array(rnf_obse.obse_nombre, ',')) n1) n2)
        WHEN rnf_obse.obse_nombre LIKE '%-%'
            THEN (SELECT
                      min(c)::INT
                      FROM
                          (SELECT n1::INT AS c FROM unnest(string_to_array(rnf_obse.obse_nombre, '-')) n1) n2)
        WHEN rnf_obse.obse_nombre LIKE 'x' THEN NULL
        ELSE rnf_obse.obse_nombre::INT
        END                                                             AS effectif_mix
  , CASE
        WHEN rnf_obse.obse_nombre LIKE '%,%'
            THEN (SELECT
  ...
  , obs_geom.geom_pt::GEOMETRY(POINT, 2154)                             AS geom
    ...
    --     WHERE
--         rnf_obse.obse_validat_choi_id <> 100375
WITH data;
--
--
...
COMMENT ON MATERIALIZED VIEW _import_serena.vm_obs_serena_detail_point IS 'Observations lisibles de la base de données SERENA';
