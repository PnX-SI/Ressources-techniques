...
CREATE MATERIALIZED VIEW IF NOT EXISTS gn_exports.mv_grilles_pag
TABLESPACE pg_default
AS
 SELECT l_areas.id_type,
    l_areas.area_name,
    l_areas.geom AS geom_grille_pag
   FROM ref_geo.l_areas
     JOIN ( SELECT st_union(l_areas_1.geom) AS pag_geom
           FROM ref_geo.l_areas l_areas_1
          WHERE l_areas_1.id_type = ANY (ARRAY[1, 20])) pag ON st_intersects(l_areas.geom, pag.pag_geom)
  WHERE l_areas.id_type = ANY (ARRAY[27, 28, 29])
WITH DATA;

ALTER TABLE IF EXISTS gn_exports.mv_grilles_pag
    OWNER TO gnpag;
...
--DROP VIEW gn_exports."v_bilan_taxo_maille10x10_PAG";

CREATE VIEW gn_exports."v_bilan_taxo_maille10x10_PAG"
 AS
  WITH ref_taxo AS (
         SELECT DISTINCT 
	 		t.cd_nom,
	...
	liste_taxons AS (
		SELECT (ref_taxo.regne::text || ref_taxo.group2_inpn::text) || mv_grilles_pag.area_name::text AS ref_geo_group,			
            ref_taxo.regne,
            ref_taxo.group2_inpn,
            mv_grilles_pag.area_name,
            ref_taxo.cd_ref,
            mv_grilles_pag.geom_grille_pag,
           ...
           FROM gn_synthese.synthese
             ...
             JOIN gn_exports.mv_grilles_pag ON st_intersects(synthese.the_geom_local, mv_grilles_pag.geom_grille_pag)
          ...
          WHERE (synthese.id_nomenclature_valid_status = ANY (ARRAY[315, 316, 458])) AND mv_grilles_pag.id_type = 27
          GROUP BY ref_taxo.regne, ref_taxo.group2_inpn, mv_grilles_pag.area_name, ref_taxo.cd_ref, mv_grilles_pag.geom_grille_pag, 
	...
	stat_mailles_nb_data as (
		SELECT (ref_taxo.regne::text || ref_taxo.group2_inpn::text) || mv_grilles_pag.area_name::text AS ref_geo_group,			
		   ...
		   FROM gn_synthese.synthese
			 ...
			 JOIN gn_exports.mv_grilles_pag ON st_intersects(synthese.the_geom_local, mv_grilles_pag.geom_grille_pag)
		  WHERE (synthese.id_nomenclature_valid_status = ANY (ARRAY[315, 316, 458])) AND mv_grilles_pag.id_type = 27
		  GROUP BY ref_taxo.regne, ref_taxo.group2_inpn, mv_grilles_pag.area_name, mv_grilles_pag.geom_grille_pag
        ),
	stat_mailles_nb_jdd AS (
		SELECT refs_jdd.ref_geo_group,			
			...
			refs_jdd.area_name,
			refs_jdd.geom_grille_pag,
			count(*) as nb_jdd
			FROM (SELECT DISTINCT (ref_taxo.regne::text || ref_taxo.group2_inpn::text) || mv_grilles_pag.area_name::text AS ref_geo_group,			
				ref_taxo.regne,
				ref_taxo.group2_inpn,
				mv_grilles_pag.area_name,
				mv_grilles_pag.geom_grille_pag,
			   ...
			   FROM gn_synthese.synthese
				 JOIN ref_taxo ON synthese.cd_nom = ref_taxo.cd_nom
				 JOIN gn_exports.mv_grilles_pag ON st_intersects(synthese.the_geom_local, mv_grilles_pag.geom_grille_pag)
			  WHERE (synthese.id_nomenclature_valid_status = ANY (ARRAY[315, 316, 458])) AND mv_grilles_pag.id_type = 27) AS refs_jdd
		  GROUP BY refs_jdd.ref_geo_group, refs_jdd.regne, refs_jdd.group2_inpn, refs_jdd.area_name, refs_jdd.geom_grille_pag
		),
		...
		SELECT refs_prospe.ref_geo_group, 
			...
			FROM (SELECT DISTINCT (ref_taxo.regne::text || ref_taxo.group2_inpn::text) || mv_grilles_pag.area_name::text AS ref_geo_group,			
			   ...
			   FROM gn_synthese.synthese
				 JOIN ref_taxo ON synthese.cd_nom = ref_taxo.cd_nom
				 JOIN gn_exports.mv_grilles_pag ON st_intersects(synthese.the_geom_local, mv_grilles_pag.geom_grille_pag)
			  WHERE (synthese.id_nomenclature_valid_status = ANY (ARRAY[315, 316, 458])) AND mv_grilles_pag.id_type = 27) as refs_prospe
 ...
 SELECT liste_refs_tot.area_name,
    ...
    liste_refs_tot.geom_grille_pag
   FROM ( SELECT liste_taxons.ref_geo_group,
            liste_taxons.area_name,
            liste_taxons.geom_grille_pag,
            liste_taxons.regne,
            liste_taxons.group2_inpn,
            count(*) AS nb_taxons_total
           FROM liste_taxons
          GROUP BY liste_taxons.ref_geo_group, liste_taxons.area_name, liste_taxons.geom_grille_pag, liste_taxons.regne, liste_taxons.group2_inpn) liste_refs_tot
  ...
  GROUP BY liste_refs_tot.area_name, liste_refs_tot.regne, liste_refs_tot.group2_inpn,
	...
	liste_refs_tot.geom_grille_pag, liste_refs_sensreg.nb_taxons_sensreg
...
ALTER TABLE gn_exports."v_bilan_taxo_maille10x10_PAG"
    OWNER TO gnpag;
COMMENT ON VIEW gn_exports."v_bilan_taxo_maille10x10_PAG"
...
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".area_name IS 'Nom de la maille'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".regne IS 'Regne'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".group2_inpn IS 'Groupe taxonomique (group2_inpn)'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".nb_data IS 'Nombre de données sur la maille pour le groupe taxonomique désigné'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".nb_jdd IS 'Nombre de jeux de données sur la maille pour le groupe taxonomique désigné'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".nb_jours_prospe IS 'Nombre de jours sur la maille pour le groupe taxonomique désigné'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".nb_taxons_total IS 'Nombre de taxons (tous niveau confondus) sur la maille pour le groupe taxonomique désigné'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".nb_taxons_a_statut IS 'Nombre de taxons ayant un statut de protection/menace/znieff/sensibilité (tous niveau confondus) sur la maille pour le groupe taxonomique désigné'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".nb_taxons_proteges IS 'Nombre de taxons à protection stricte (tous niveau confondus) sur la maille pour le groupe taxonomique désigné'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".nb_taxons_znieff IS 'Nombre de taxons déterminants ZNIEFF (tous niveau confondus) sur la maille pour le groupe taxonomique désigné'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".nb_taxons_menaces_mond IS 'Nombre de taxons menacés mondialement (tous niveau confondus) sur la maille pour le groupe taxonomique désigné'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".nb_taxons_menaces_reg IS 'Nombre de taxons menacés au niveau régional (tous niveau confondus) sur la maille pour le groupe taxonomique désigné'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".nb_taxons_sensreg IS 'Nombre de taxons sensibles (tous niveau confondus) sur la maille pour le groupe taxonomique désigné'; 
COMMENT ON COLUMN gn_exports."v_bilan_taxo_maille10x10_PAG".geom_grille_pag IS 'Geometrie'; 
