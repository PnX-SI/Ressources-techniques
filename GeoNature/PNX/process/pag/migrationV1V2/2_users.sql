...
--NOTICE: truncate cascades to table "cor_dataset_actor" --NOTICE: truncate cascades to table "t_validations" 
...
--NOTICE: truncate cascades to table "cor_counting_occtax" TRUNCATE utilisateurs.t_listes CASCADE; 
--NOTICE: truncate cascades to table "cor_role_liste" TRUNCATE utilisateurs.t_applications CASCADE; 
--NOTICE: truncate cascades to table "cor_profil_for_app" 
--NOTICE: truncate cascades to table "cor_role_app_profil" 
--NOTICE: truncate cascades to table "cor_application_nomenclature" TRUNCATE utilisateurs.cor_role_app_profil CASCADE; 
...
INSERT INTO utilisateurs.bib_organismes(
  ...
  id_organisme
  --,uuid_organisme,  id_parent 
  FROM v1_compat.bib_organismes WHERE id_organisme NOT IN (SELECT id_organisme FROM utilisateurs.bib_organismes); 
...
UPDATE utilisateurs.t_roles SET uuid_role = uuid_generate_v4() WHERE uuid_role IS NULL; 
INSERT INTO utilisateurs.cor_roles (id_role_groupe, id_role_utilisateur) 
	SELECT * FROM v1_compat.cor_roles; 
echo "Attention! La v1.9 n'avait ni id_parent ni code_application parmis les t_applications!"	
INSERT INTO utilisateurs.t_applications (id_application, nom_application, desc_application, code_application)--, id_parent) 
	SELECT id_application, nom_application, desc_application, 'GN1.9'
	FROM v1_compat.t_applications 
	WHERE id_application NOT in (SELECT id_application FROM utilisateurs.t_applications); 
	
-------------- IL N'Y AVAIT PAS DE t_listes dans la 1.9!
-- Deja créé avec le 1_create_compatv1to2.sql ??  ou compléments à ajouter? ----- INSERT INTO utilisateurs.t_listes(id_liste, code_liste, nom_liste, desc_liste) SELECT * FROM v1_compat.t_listes; 
INSERT INTO utilisateurs.t_listes (id_liste, code_liste, nom_liste, desc_liste)
	...
	FROM v1_compat.t_menus;  -- insertion des listes observateurs faune (id 9 pour appli 14 GN19) et flore (id 10 pour appli 14 GN19)
INSERT INTO utilisateurs.cor_role_liste (id_role, id_liste) 
	SELECT id_role, id_menu
	FROM v1_compat.cor_role_menu;  --Attention à tout réviser
INSERT INTO utilisateurs.cor_profil_for_app (id_profil, id_application) SELECT * FROM v1_compat.cor_profil_for_app; 
INSERT INTO utilisateurs.cor_role_app_profil (id_role, id_application, id_profil) 
	SELECT * FROM v1_compat.cor_role_app_profil; 
UPDATE utilisateurs.cor_role_app_profil
	SET id_application = 3 WHERE id_application =14;  -- On redirige l'id_app 14 (GN1.9) vers le 3 (GN2.5)
 ...
 SELECT t_listes.id_liste AS id_menu,
  ...
  (SELECT DISTINCT r.id_role,
   ...
   WHERE (r.id_role IN ( 
	...
	OR (r.id_role IN (SELECT crm.id_role
           FROM utilisateurs.cor_role_menu crm
...
-- Suppression des droits de l'application GeoNature DELETE FROM utilisateurs.cor_role_app_profil WHERE id_application = (
...
--  FROM v1_compat.cor_role_droit_application cor JOIN v1_compat.t_applications app ON app.id_application = cor.id_application 
--	WHERE app.code_application = 'GN' AND cor.id_droit >= 1;

  INSERT INTO utilisateurs.cor_role_app_profil 
  SELECT cor_role_droit_application.id_role, 3, 1 
  FROM v1_compat.cor_role_droit_application JOIN v1_compat.t_applications ON t_applications.id_application = cor_role_droit_application.id_application 
	WHERE t_applications.id_application = '14' AND cor_role_droit_application.id_droit >= 1;