...
CREATE VIEW atlas.syntheseff AS
WITH areas AS (
	...
	SELECT s.id_synthese,
	   ...
	   LEFT OUTER JOIN ref_nomenclatures.t_nomenclatures dl ON s.id_nomenclature_diffusion_level = dl.id_nomenclature
	   LEFT OUTER JOIN ref_nomenclatures.t_nomenclatures st ON s.id_nomenclature_observation_status = st.id_nomenclature
 ...
 SELECT d.id_synthese,
    ...
    diffusion_level
FROM obs_data d
LEFT JOIN atlas.l_communes c ON st_intersects(d.the_geom_point, c.the_geom);
