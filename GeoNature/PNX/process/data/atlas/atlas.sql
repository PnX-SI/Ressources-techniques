...
INSERT_ALTITUDE
UPDATE atlas.bib_altitudes set label_altitude = '_' || altitude_min || '_' || altitude_max+1 WHERE label_altitude IS NULL;
...
WITH geomdata AS (
  SELECT ST_UNION(the_geom_point) as geom from atlas.syntheseff
)
SELECT c.insee,
c.commune_maj,
c.the_geom,
...
JOIN geomdata on ST_INTERSECTS(geomdata.geom, c.the_geom)
