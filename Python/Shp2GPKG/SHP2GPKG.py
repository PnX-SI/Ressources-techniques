    ...
    for fichier in liste:
        if os.path.isdir(os.path.join(repertoire, fichier)):  # Si on trouve un dossier
            ...
            if fichier[-4:].lower() == ".shp":  # On cherche à savoir si le fichier est un shapefile
                ...
                with fiona.open(os.path.join(repertoire, fichier)) as src:  # Ouverture du shapefile

                    meta = src.meta.copy()

                    if (
                        meta["schema"]["geometry"][:5] != "Multi"
                        and meta["schema"]["geometry"][:2] != "3D"
                    ):  # Si l'entité n'est pas multi... ni en 3D

                        meta["schema"]["geometry"] = (
                            "Multi" + meta["schema"]["geometry"]
                        )  # On ajoute le multi au debut

                    elif meta["schema"]["geometry"][:2] == "3D":  # Si on est en 3D

                        meta["schema"]["geometry"] = (
                            "Multi" + meta["schema"]["geometry"][3:]
                        )  # On remplace le 3D par Multi

                    meta["driver"] = "GPKG"  # On prepare le futur fichier en Geopackage

                    with fiona.open(
                        os.path.join(repertoire, layerFinal),
                        "w",
                        **meta
                    ) as dst:  # Creation du fichier geopackage

                        for f in src:

                            # Fiona >= 1.9 : creation d'une copie mutable du feature
                            feature = {
                                "type": "Feature",
                                "id": f.get("id"),
                            ...
                            if "MultiPolygon" in meta["schema"]["geometry"]:  # Si on a une couche de polygones
                                ...
                                if str(feature["geometry"]["coordinates"])[:3] == "[[[":  # Si l'entité est un multipolygone

                                    feature["geometry"] = {
                                        "type": meta["schema"]["geometry"],  # On donne le bon type de geometrie
                                        "coordinates": feature["geometry"]["coordinates"],
                                    }  # On affecte les coordonnees
                                    ...
                                    feature["geometry"] = {
                                        ...
                                        "coordinates": [feature["geometry"]["coordinates"]],
                                    }  # On rajoute un niveau de [] correspondant au multi

                                dst.write(feature)

                            elif "MultiLineString" in meta["schema"]["geometry"]:  # Si l'entité est une ligne

                                if str(feature["geometry"]["coordinates"])[:2] == "[[":  # Si c'est une multi-ligne
                                    ...
                                    feature["geometry"] = {  # Si c'est une ligne normale
                                        "type": meta["schema"]["geometry"],
                                        "coordinates": [feature["geometry"]["coordinates"]],
                                    }  # On rajoute un niveau de [] correspondant au multi
                            ...
                            elif str(feature["geometry"]["coordinates"])[:1] == "[":  # Si c'est un multi-point
                                ...
                                feature["geometry"] = {  # Si c'est un point
                                    "type": meta["schema"]["geometry"],
                                    "coordinates": [feature["geometry"]["coordinates"]],
                                }  # On rajoute un niveau de [] correspondant au multi
...
SHP2GPKG('/Users/mathiaspisch/Documents/ENSAE/TRAVAIL/1A/Samuel R/Mission Peter Howe/mission-peter-howe/R output Grid')
