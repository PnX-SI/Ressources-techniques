...
Sur le pc, localement, créer un fichier .pg_service.conf qui sera enregistré dans le home de l’utilisateur (`~/.pg_service.conf`) (ou` %APPDATA%\postgresql\pg_service.conf` pour windows – non testé!).
...
le changer de plan ou ajouter la permission is_external_db_supported
